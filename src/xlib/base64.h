...
#ifndef XLIB_BASE64_H_
#define XLIB_BASE64_H_
...
std::size_t encode(void* dest, void const* src, std::size_t len) {
    ...
    auto const tab = xlib::get_alphabet();
...
std::pair<std::size_t, std::size_t> decode(void* dest, char const* src, std::size_t len) {
    ...
    auto const inverse = xlib::get_inverse();
...
base64_encode(std::uint8_t const* data, std::size_t len) {
    std::string dest;
    dest.resize(xlib::encoded_size(len));
    dest.resize(xlib::encode(&dest[0], data, len));
    return dest;
}
...
template<class = void>
std::string Base64Decode(std::string const& data) {
    std::string dest;
    dest.resize(xlib::decoded_size(data.size()));
    auto const result = xlib::decode(
